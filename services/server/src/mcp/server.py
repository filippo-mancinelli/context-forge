...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "then repo_get_file to read a whole file; repo_symbols to look up a "
        "function/class/method/type definition by name (lexical, not semantic — use "
        "it instead of repo_search when you already know the identifier); "
        "repo_references to find all usages of a symbol; code_explain to get an AI "
        "explanation of code; repo_annotate/repo_annotations for persistent notes on "
        "files; repo_index to (re)index; repo_relationships for cross-repo similarity\n"
        ...
        "db_describe / db_query. Call db_list() first to see connection names; "
